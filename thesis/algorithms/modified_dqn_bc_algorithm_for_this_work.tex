...
\STATE Initialize Q-network $Q_\theta$, target network $Q_{\theta'}$ with $\theta' \leftarrow \theta$, and state normalization model $\phi$.
\FOR{$t = 1$ to $T$}
    \STATE Sample mini-batch $\mathcal{M}$ of $N$ transitions $(s,a,r,s')$ from $\mathcal{B}$.
    \STATE Compute normalized states: $\tilde{s} = \phi(s)$, $\tilde{s}' = \phi(s')$.
