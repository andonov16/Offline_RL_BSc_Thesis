...
The exact formulation of the algorithm proposed in \cite{BCQ_benchmarking} is shown in Algorithm~\ref{alg:bcq_original}. In the original work, the method is referred to as BCQ. BCQ combines DQN with a generative model that approximates the behaviour policy and restricts the set of candidate actions during Q-value updates. Both the DQN and behaviour policy are trained simultaneously. 


In this work, we use the same general principle of constraining the action space using a behaviour policy models. However, instead of training a generative model jointly with the Q-network, we employ a separately trained BC classifier as the behaviour policy estimator. The pretrained BC model is then used during offline training to regularize action selection.

To clearly distinguish the approach used in this work from the original BCQ formulation, we refer to all algorithms that combine a DQN agent with a pretrained BC-based regularization mechanism as DQN+BC. This terminology emphasizes the two main components of the method: the Q-learning network and the pretrained BC model used to guide the policy during offline training while distinguishing it from the original approach defined by \cite{BCQ_benchmarking}.
...
\section{The BCQ Algorithm}
\label{sec:the_DQN_BC_algorithm}

A BCQ agent learns a Q-network that takes a given state $s$ as input and outputs Q-values for all possible discrete actions in that state.
...
\input{algorithms/original_dqn_bc_algorithm}
...
\input{algorithms/modified_dqn_bc_algorithm_for_this_work}
